...
const Blog = () => {
  ...
  return (
    <div className="min-h-screen bg-[#FDFDFD] dark:bg-black text-[#0F172A] dark:text-green-600 	px-5 sm:px-6 md:px-10 py-10 sm:py-12 font-mono">
      {/* Terminal Heading */}
      <div className="max-w-5xl mx-auto mb-10 md:pt-10">
        <motion.h2 className="text-3xl sm:text-3xl md:text-4xl font-bold text-[#6366F1] dark:text-green-600 terminal-blink" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1 }} >
        ...
        {blogs.map((item, index) => (
          <Tilt key={index} glareEnable glareMaxOpacity={0.2} scale={1.05} transitionSpeed={400} className="bg-white dark:bg-[#0f0f0f] relative overflow-hidden rounded-2xl shadow-2xl backdrop-blur-lg border border-green-700/50">
            <motion.div key={index} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }} className=" p-4 rounded-lg hover:shadow-green-400/30 transition-all cursor-pointer " >
              <img src={item.coverImage?.url} alt={item.title} className="w-full h-40 object-cover mb-4 dark:border dark:border-green-700/40 rounded" />
