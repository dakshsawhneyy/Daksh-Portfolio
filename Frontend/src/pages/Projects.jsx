...
const Projects = () => {
  ...
  const categories = ['All', ...new Set(list.flatMap(project => project.category || []))].slice(0, 7)
  const visible = list.filter(project => { const text = `${project.title} ${project.description} ${(project.tags || []).join(' ')}`.toLowerCase(); return (filter === 'All' || project.category?.includes(filter)) && text.includes(query.toLowerCase()) })
