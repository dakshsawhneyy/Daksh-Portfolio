...
const Contact = () => {
  return (
    <div className="min-h-screen flex flex-col lg:flex-row bg-gradient-to-br from-gray-100 to-white dark:from-gray-900 dark:to-bgDark">
      ...
      {/* Right Panel: Form */}
      <motion.form action="https://formsubmit.co/dakshsawhneyy@gmail.com" method="POST" target="_blank" className="lg:w-1/2 p-8 lg:p-16 m-4 lg:m-8 backdrop-blur-xl bg-white/30 dark:bg-gray-800/40 rounded-tl-3xl rounded-bl-3xl shadow-xl" initial={{ x: 100, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ duration: 0.6 }}>
        <input type="hidden" name="_captcha" value="false" />
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <input name="name" type="text" placeholder="Your Name" required className="w-full px-4 py-3 rounded-lg bg-white dark:bg-gray-900 border dark:text-white border-gray-300 dark:border-gray-600 focus:outline-primary transition" />
          <input name="email" type="email" placeholder="Your Email" required className="w-full px-4 py-3 rounded-lg bg-white dark:bg-gray-900 border dark:text-white border-gray-300 dark:border-gray-600 focus:outline-primary transition" />
        </div>
        <textarea name="message" rows="5" placeholder="Your Message" required className="w-full mt-4 px-4 py-3 rounded-lg bg-white dark:bg-gray-900 border dark:text-white border-gray-300 dark:border-gray-600 focus:outline-primary transition"></textarea>
